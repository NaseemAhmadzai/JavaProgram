...
class Fraction(object):
    def __init__(self, str_fraction):
        ...
        frac = str_fraction.split("/")
        self.num = eval(frac[0])
        self.denom = eval(frac[1])
    ...
    def reduce(self):
        ...
        '''
        gcf = self.find_gcf(abs(self.num), abs(self.denom))
        charge = 1
        if self.num / self.denom <= 0:
            charge = -1
        return Fraction(f"{abs(self.num // gcf) * charge}/{abs(self.denom // gcf)}")
    ...
    def get_reciprocal(self):
        ...
        '''
        return Fraction(f"{self.denom}/{self.num}")
    ...
    def __add__(self, f2):
        ...
        new_denom = f2.denom * self.denom
        new_num = f2.denom * self.num + f2.num * self.denom

        frac = Fraction(f"{new_num}/{new_denom}")
    ...
    def __truediv__(self, f2):
        ...
        new_num = self.num * f2.denom
        new_denom = self.denom * f2.num
    ...
    def __eq__(self, f2):
        ...
        if f2.num == reduced_self.num and f2.denom == reduced_self.denom:
            return True
        else:
            return False

    def __lt__(self, f2):
        if self.num * f2.denom < self.denom * f2.num:
            return True
        else:
            return False

    def __le__(self, f2):
        if self.__lt__(f2) or self.__eq__(f2):
            return True
        else:
            return False
    ...
    def __ge__(self, f2):
        if self.__gt__(f2) or self.__eq__(f2):
            return True
        else:
            return False
...
if __name__ == '__main__':
    ...
    print("Testing (-3/7)==(-3/7). Expected output should be:\nTrue")
    eq = Fraction("-3/7") == Fraction("-3/7")
    print("Actual Result:")
    print(eq)

    print("Testing (1/2)<(3/4). Expected output should be:\nTrue")
    lt = Fraction("1/2") < Fraction("3/4")
    print("Actual Result:")
    print(lt)

    print("Testing (1/2)>(3/4). Expected output should be:\nFalse")
    gt = Fraction("1/2") > Fraction("3/4")
    print("Actual Result:")
    print(gt)

    print("Testing (1/2)<=(1/2). Expected output should be:\nTrue")
    le = Fraction("1/2") <= Fraction("1/2")
    print("Actual Result:")
    print(le)

    print("Testing (1/2)>=(3/4). Expected output should be:\nFalse")
    ge = Fraction("1/2") >= Fraction("3/4")
    print("Actual Result:")
    print(ge)
